...
def main():
    ...
    for j in range(len(result_prepro_test)):
        if(len(result_prepro_test[j])==1):
            for l in range(len(detailed_words_positif)):
                if(result_prepro_test[j][0] == detailed_words_positif[l][0]):
                    allpwcpos = allpwcpos * detailed_words_positif[l][2]
                elif(result_prepro_test[j][0] != detailed_words_positif[l][0]):
                    allpwcpos = 0
            for l in range(len(detailed_words_negatif)):
                if(result_prepro_test[j][0] == detailed_words_negatif[l][0]):
                    allpwcneg = allpwcneg * detailed_words_negatif[l][2]
                elif(result_prepro_test[j][0] != detailed_words_negatif[l][0]):
                    allpwcneg = 0
        elif(len(result_prepro_test[j])>1):
