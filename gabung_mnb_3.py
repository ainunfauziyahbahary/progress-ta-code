...
def testing_sentiment(data_test,prior_pos,prior_neg,detailed_words_pos,detailed_words_neg, sent_test):
    ...
    chooseclasspos = 1
    chooseclassneg = 1
    # tanggapan_test = preprocessing_uselist(data_test)
    for j in range(len(data_test)):
        if(len(data_test[j])!=0):
            #look for all P(w|c)
            for k in range(len(data_test[j])):
                for l in range(len(detailed_words_pos)):
                    if(data_test[j][k] == detailed_words_pos[l][0]):
                        allpwcpos = allpwcpos * detailed_words_pos[l][2]
                    else:
                        continue
            for m in range(len(data_test[j])):
                for n in range(len(detailed_words_neg)):
                    if(data_test[j][m] == detailed_words_neg[n][0]):
            ...
            if (chooseclasspos > chooseclassneg):
                detailed_test = data_test[j] , round(chooseclasspos,5), 'POSITIVE'
                result_sent_test.append('1')
            elif (chooseclasspos < chooseclassneg):
                detailed_test = data_test[j] , round(chooseclassneg,5),'NEGATIVE'
