...
def pisah_list_kelas(filename): #ganti jadi untuk membaca list
    with open(filename) as f:
        reader = csv.reader(f, delimiter=',')
        for row in reader:
            if row[1] == '0':
...
def main():
    ...
    for train_index, test_index in ss.split(all_data):
        ...
        for i in range(len(train_index)):
            train_data = all_data["Tanggapan"].values[train_index[i]], all_data["S_SI"].values[train_index[i]]
            print(all_data["Tanggapan"].values[train_index[i]], all_data["S_SI"].values[train_index[i]])
            all_train_data.append(tuple(train_data))
        ...
        print("ini Test Data = ", all_test_data)
